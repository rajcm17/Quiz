    ...
    const fetchQuizzes = async () => {
      if (user) {
        if (user.role === 'admin') {
          ...
          // Users see all published quizzes to attend
          ...
          if (result.success) {
            ...
            setStats({
              ...
              totalDrafts: 0,
              totalPublished: result.quizzes.length
            });
          }
        }
        setLoading(false);
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    <dt className="text-sm font-medium text-gray-500 truncate">Ready to Take</dt>
                    <dd className="flex items-baseline">
                      <div className="text-2xl font-semibold text-gray-900">{stats.totalPublished}</div>
                    </dd>
                  </dl>
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-semibold text-gray-900">
            {isAdmin ? 'Your Quizzes' : 'Available Quizzes'}
          </h2>
          {isAdmin && (
            <Link
              to="/create-quiz"
      ...
      {quizzes.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">
            {isAdmin ? 'Get started by creating a new quiz.' : 'No quizzes available yet.'}
          </p>
          {isAdmin && (
            <div className="mt-6">
              <Link
